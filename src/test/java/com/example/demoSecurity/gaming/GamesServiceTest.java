...
//    }
    ...
    class playRockPaperScissors {
        @Test
        void whenPlayerChooseRock() {
            String result = gamesService.playRockPaperScissors(1);
            if (gamesService.getPc().equals("Rock"))
                assertEquals("It's a draw", result);
            else if (gamesService.getPc().equals("Paper"))
                assertEquals("The winner is PC", result);
            else if (gamesService.getPc().equals("Scissor"))
                assertEquals("The winner is Player", result);
        }
        @Test
        void whenPlayerChoosePaper() {
            String result = gamesService.playRockPaperScissors(2);
            if (gamesService.getPc().equals("Rock"))
                assertEquals("The winner is Player", result);
            else if (gamesService.getPc().equals("Paper"))
                assertEquals("It's a draw", result);
        ...
        @Test
        void whenPlayerChooseScissor() {
            String result = gamesService.playRockPaperScissors(3);
            if (gamesService.getPc().equals("Rock"))
                assertEquals("The winner is Pc", result);
            else if (gamesService.getPc().equals("Paper"))
                assertEquals("The winner is Player", result);
